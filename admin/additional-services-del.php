...
if(count($arr)>0){
...

	{
	    ...
	    $sql="delete from $tbl_additional_services where id in ($str_rest_refs)"; 
	...
	{	
		...
		$sql="update $tbl_additional_services set status=1 where id in ($str_rest_refs)";
	...
	{		
		 ...
		 $sql="update $tbl_additional_services set status=0 where id in ($str_rest_refs)";
