...
if(count($arr)>0){
	...
	if($Submit=='Delete')
	{
	    $sql="delete from $tbl_member where id in ($str_rest_refs)"; 
		$obj->query($sql);
	...
	{	
		$sql="update $tbl_member set status=1 where ID in ($str_rest_refs)";
		$obj->query($sql);
	...
	{		
		 $sql="update $tbl_member set status=0 where ID in ($str_rest_refs)";
		$obj->query($sql);
