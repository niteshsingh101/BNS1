...
if(count($arr)>0){
...

	{
	    ...
	    $sql="delete from tbl_origin where id in ($str_rest_refs)"; 
	...
	{	
		...
		$sql="update tbl_origin set status=1 where id in ($str_rest_refs)";
	...
	{		
		 ...
		 $sql="update tbl_origin set status=0 where id in ($str_rest_refs)";
