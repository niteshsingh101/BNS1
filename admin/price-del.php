...
if(count($arr)>0){
...

	{
	    ...
	    $sql="delete from tbl_price where id in ($str_rest_refs)"; 
	...
	{	
		...
		$sql="update tbl_price set status=1 where id in ($str_rest_refs)";
	...
	{		
		 ...
		 $sql="update tbl_price set status=0 where id in ($str_rest_refs)";
