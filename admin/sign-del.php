...
if(count($arr)>0){
...

	{
	    ...
	    $sql="delete from $tbl_sign where id in ($str_rest_refs)"; 
	...
	{	
		...
		$sql="update $tbl_sign set status=1 where id in ($str_rest_refs)";
	...
	{		
		 ...
		 $sql="update $tbl_sign set status=0 where id in ($str_rest_refs)";
