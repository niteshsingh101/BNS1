...
if(count($arr)>0){
...

	{
	    ...
	    $sql="delete from $tbl_country where id in ($str_rest_refs)"; 
	...
	{	
		...
		$sql="update $tbl_country set status=1 where id in ($str_rest_refs)";
	...
	{		
		 ...
		 $sql="update $tbl_country set status=0 where id in ($str_rest_refs)";
